...
(function () {
  ...
  /* Un mot-clé doit commencer sur une frontière de mot. Sans cette contrainte,
     la clé « cc » était trouvée à l'intérieur d'« ACCI » : la suggestion
     « Comment adhérer à l'ACCI ? » déclenchait la réponse de salutation.
     Les clés se terminant par un radical (« adher », « harcel ») restent des
     préfixes volontaires — seul le DÉBUT est ancré. */
  function keyMatches(q, k) {
    k = norm(k);
    var i = q.indexOf(k);
    while (i !== -1) {
  ...
  function findIntent(text) {
    ...
    INTENTS.forEach(function (it) {
      ...
      it.keys.forEach(function (k) {
        // Une clé longue est un signal plus fiable qu'une clé de 2-3 lettres.
        if (keyMatches(q, k)) s += k.length >= 5 ? 3 : 1;
      });
